...
export class Migration20251111112831_init_profile_tables extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "country" ("code" text not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "alpha3" text not null, "name" text not null, "country_code" text not null, "region" text not null, "region_code" text not null, constraint "country_pkey" primary key ("code"));`,
    );

    this.addSql(
      `create table "city" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" text not null, "code" text not null);`,
    );

    this.addSql(
      `create table "guide" ("id" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" text not null, "bio" text not null, "country_code" text not null, "city_id" int not null, constraint "guide_pkey" primary key ("id"));`,
    );
    this.addSql(
    ...
    this.addSql(
      `alter table "city" add constraint "city_code_foreign" foreign key ("code") references "country" ("code") on update cascade;`,
    );
