...
export class RefreshTokenService {
  ...
  async storeRefreshToken(
    ...
    try {
      ...
      this.logger.debug(
        `Stored refresh token in Redis: ${key} for user: ${userData.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to store refresh token in Redis: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async validateRefreshToken(
    ...
    try {
      ...
      this.logger.debug(
        `Validated refresh token for user: ${tokenData.userId}`,
      );
      return tokenData;
    } catch (error) {
